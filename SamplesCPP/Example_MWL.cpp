...
#include "..\QMediToolsCore\QMediToolsCore.h"

// Query the SCP and dump some data from response
static HRESULT do_mwl_find(HQMTSCU hScu, HQMTMSG hm)
{
...
static HRESULT do_mwl_find(HQMTSCU hScu)
{
	HRESULT hr = QMTNegotiateAssociation(hScu, L"MWL-FIND");
	if (SUCCEEDED(hr)) {
		HQMTMSG hm = QMTCreateFindMessage(L"MWL", NULL /* default="MWLQUERY" */);
		if (hm) {
			do_mwl_find(hScu, hm);
			QMTFreeMessage(hm);
		}
